"""africas2s.combine_terciles — generic tercile-forecast combination.
...
def test_equal_weight_average_and_simplex():
    ...
    out = africas2s.combine_terciles([a, b])
...
def test_weights_renormalized():
    ...
    out = africas2s.combine_terciles([a, b], weights=[3, 1])   # 0.75 / 0.25
...
def test_hierarchy_component_equal_matches_manual():
    ...
    exp1 = africas2s.combine_terciles([_probs(LAT, LON, (0.7, 0.2, 0.1)),
                                       _probs(LAT, LON, (0.5, 0.3, 0.2))])
    exp2 = _probs(LAT, LON, (0.2, 0.3, 0.5))
    exp3 = _probs(LAT, LON, (0.3, 0.4, 0.3))
    obj = africas2s.combine_terciles([exp1, exp2, exp3])
...
def test_regrid_to_common_grid():
    ...
    out = africas2s.combine_terciles([coarse, fine], regrid_to=coarse)
...
def test_accepts_latitude_longitude_dim_aliases():
    ...
    rest of africas2s, rather than requiring lat/lon (it previously KeyError'd)."""
    def _aliased(fill):
        a = np.empty((3, len(LAT), len(LON)))
        for k in range(3):
            a[k] = fill[k]
    ...
    out = africas2s.combine_terciles([_aliased((0.6, 0.3, 0.1)), _aliased((0.2, 0.3, 0.5))])
...
def test_rejects_bad_weights():
    ...
    with pytest.raises(ValueError):
        africas2s.combine_terciles([a, a], weights=[1])          # wrong length
    with pytest.raises(ValueError):
        africas2s.combine_terciles([a, a], weights=[0, 0])       # all zero
...
def test_climatology_fill_keeps_full_divisor():
    ...
    out = africas2s.combine_terciles([a, b], missing="climatology")
...
def test_climatology_fill_pulls_toward_flat_not_away():
    ...
    dropped = africas2s.combine_terciles([a, b])
    filled = africas2s.combine_terciles([a, b], missing="climatology")
...
def test_all_components_absent_stays_nan_under_both():
    ...
    for mode in ("drop", "climatology"):
        out = africas2s.combine_terciles([a, b], missing=mode)
...
def test_climatology_fill_respects_weights():
    ...
    b.loc[dict(lat=0.0, lon=10.0)] = np.nan
    out = africas2s.combine_terciles([a, b], weights=[3, 1], missing="climatology")
    np.testing.assert_allclose(out.sel(tercile=0, lat=0.0, lon=10.0).values,
                               0.75 * 0.8 + 0.25 * (1 / 3))
...
def test_rejects_bad_missing_mode():
    a = _probs(LAT, LON, (0.5, 0.3, 0.2))
    with pytest.raises(ValueError):
        africas2s.combine_terciles([a, a], missing="climatological")
