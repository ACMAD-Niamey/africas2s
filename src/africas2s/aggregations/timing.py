...
try:
    ...
    _VERSION = _pkg_version("africas2s")
...
class TimingResult:
    ...
    :func:`africas2s.time.season_step`. It is leap-safe and works for seasons
    ...
    africas2s takes one. Handle the season-failure years before converting it
    with :func:`africas2s.to_tercile_cv`, which reads NaN as missing data:
...
def _provenance(params: dict, season, *, definition_of: str | None = None) -> dict:
    ...
    out["africas2s_version"] = _VERSION
...
_SHARED_PROVENANCE = ("season", "africas2s_version")
...
def _require_daily(daily, time_dim: str) -> None:
    ...
    if cadence != "daily":
        raise ValueError(
            f"africas2s.aggregations needs daily rainfall; the {time_dim!r} axis "
...
def onset(
    ...
    *,
    ...
    daily : xr.DataArray
        ...
        that span warns.
    season : see :func:`africas2s.time.season_bounds`
...
def season_length(onset: TimingResult, cessation: TimingResult) -> xr.DataArray:
    ...
    ``season`` and ``africas2s_version`` are recorded once, from ``onset``.
    """
