...
:class:`~africas2s.analog.AnalogSet`, however it was selected — nine El Niño
...
def complete(
    ...
    *,
    ...
    analogs : AnalogSet
        Which years to complete with. See :mod:`africas2s.analog`.
    climatology : xr.DataArray
        ``(year, step, ...)`` season-aligned historical increments — the output
        of :func:`africas2s.climate.seasonal_stack`. Supplies the analog
        remainders and, by default, the reference distribution for
        ``percentile``.
    season, year : see :func:`africas2s.time.season_bounds`
    ...
    for name, da in (("climatology", climatology), ("analog_source", analog_source)):
        if da is not None and not {"year", "step"} <= set(da.dims):
            raise ValueError(
                ...
                f"africas2s.climate.seasonal_stack); got {tuple(da.dims)}"
            )
