...
_DEFAULT_CACHE_DIR = Path.home() / ".cache" / "africas2s"
...
def _read_tile(url):
    ...
    try:
        ...
        if r.status_code == 404:
            return None
        r.raise_for_status()
        with tempfile.TemporaryDirectory(prefix="africas2s-glo90-") as tmp:
...
def load_terrain(like, cache_dir=None):
    ...
    *block-averaged* onto each grid cell
    ...
    cache_dir : path-like, optional
        Directory for the NetCDF cache. Default: ``~/.cache/africas2s``.
