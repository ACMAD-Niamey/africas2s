...
Two distinct paths in `africas2s.tercile`. Choosing the wrong one silently inflates skill.
...
**Leave-one-out CV helpers** (`africas2s.metrics.cross_validation`, re-exported at top level except `leverage`):
...
africas2s.metrics.leverage(x)           # per-point leverage h_i = 1/n + (x-x̄)²/Sxx (module-qualified only)
```
...
**Significance** (`africas2s.metrics.significance`, re-exported at top level):
