# Daily aggregations (`africas2s.aggregations`)
...
Reference this module-qualified, following `africas2s.time` and
`africas2s.tercile`. Its functions are not in the top-level `__all__`:
...
forecasts take the same path. `season` is anything `africas2s.time.season_bounds`
...
`step` is on the same axis as `africas2s.time.season_step`: leap-safe, and
...
Input must be daily. All three functions call `africas2s.time.infer_cadence` on
...
`africas2s_version` are always included, alongside each function's criterion
parameters:
...
other. `season` and `africas2s_version` describe the analysis rather than
...
| `season`, `africas2s_version` | shared, recorded once |
