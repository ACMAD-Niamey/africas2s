...
**smoothed_regression** — Kharin et al. (2017) smoothed-coefficient postprocessing; season-aware: `predictor` `(season, year, member, lat, lon)`, `obs` `(season, year, lat, lon)`, same grid. Also accepts the ereg-style `{model: (hindcast, forecast)}` dict (or `{model: hindcast}` + `forecast={model: forecast}`): members are pooled across models into one super-ensemble with reindexed member ids (not per-model averaged like ereg), hindcast years intersected across models. Extra kwargs: `output_type="deterministic"|"tercile"` (default deterministic → `(season, lat, lon)`; tercile → `(season, tercile, lat, lon)`), `temporal_sigma=None|float|"constant"` (per-season / cyclic Gaussian across the seasonal cycle / one year-round coefficient), `distribution="normal"|"gamma"` (tercile only; gamma maps members+obs through the gamma CDF for non-negative variables), `constrained=True` (tercile only; analytic spread vs `False` CRPS-minimizing). Target selection, exactly one of (both raises `ValueError`): `forecast_year` (hindcast-mode; must be a year present in the hindcast) or `forecast=` (out-of-sample ensemble `(season, member, lat, lon)`, singleton `year` dim tolerated; `member` required for tercile, optional for deterministic; applied through the hindcast-fitted coefficients, gamma parameters, and tercile boundaries). The fit itself always comes from the hindcast, aligned to the obs years (extra hindcast years are trimmed, not an error). Tercile output without a `member` dim raises `ValueError`. Score deterministic output with `msss`, tercile output with `crpss`/`reliability`.
...
**`Index.custom(*, name, regions, combine, transform=..., weights=..., baseline=...)`** — `regions` values may each be a bbox list `[lat_s, lat_n, lon_w, lon_e]`, a `{"south":…, "north":…, "west":…, "east":…}` mapping, or the **name of a box in `africas2s.indices.REGIONS`**. Shapefile paths / shapely geometries work only if Rosetta is installed. `combine` receives the transformed regional series keyed by region name, e.g. WVG is `lambda z: z["nino34"] - (z["wnp"] + z["wep"] + z["wsp"]) / 3`.

Box definitions live in `africas2s.indices.REGIONS` (a dict of `{south, north, west, east}` in 0–360 lon; `reduce` normalizes whatever convention the field uses). Neither `REGIONS` nor `Index.list_named` is in the top-level `__all__`.
...
Per-gridpoint ensemble-mean regression slope `a = Cov(Fbar, O)/Var(Fbar)` as a function of the seasonal cycle. Inputs `(season, year, member, lat, lon)` / `(season, year, lat, lon)` on the same grid → coefficient `(season, lat, lon)`. `temporal_sigma`: `None` (per-season, unsmoothed), a float (cyclic Gaussian smoothing across the season axis), or `"constant"` (a single pooled time-invariant slope — a pooled regression over all seasons, not the large-sigma limit). The probabilistic companion functions live in `africas2s.methods.smoothed_regression` — see `references/methods.md`.
...
- `africas2s.time.{season_step, season_bounds, season_months, season_times, infer_cadence, dekad_start_for, dekad_window, dekad_of_year, dekads_for_issuance, pentad_start_for, pentad_window}` — season-step alignment + dekad/pentad calendar arithmetic. Reference module-qualified (`africas2s.time.season_step`). See `references/analog-completion.md`.
- `africas2s.indices.REGIONS` (box definitions dict) and `africas2s.indices.Index.list_named()` — public, module-qualified only.
- `africas2s.metrics.leverage(x)` — simple-regression leverage `h_i`, alongside the top-level `loo_predict`/`loo_corr`; see `references/metrics-and-terciles.md`.
- `africas2s.tercile.{to_tercile, to_tercile_cv, cpt_tercile_forecast}` — see `references/metrics-and-terciles.md`.
- `africas2s.cv.{loyo, lko, blocked, expanding, get_cv}` — see `references/methods.md`.
- `africas2s.registry.{register_method, register_calibrator, register_metric, register_strategy, get_method, get_metric, get_strategy, ...}` — extension points.
- `africas2s.methods.base.{MethodBase, ProbabilisticMethodBase}` — subclass + `register_method` to add a method; `ProbabilisticMethodBase` adds `predict_distribution()`.
- `africas2s.logistic.logistic_forecast(...)` — low-level per-cell logistic engine.
- `africas2s.metrics.spread_error.spread_error_diagnostics(forecast, obs, *, spatial=False)`.
- `africas2s.methods.smoothed_regression.{fit_gamma, gamma_to_normal, normal_to_gamma, fit_ab, fit_ab_field, smooth_ab, normal_category_probs}` — the Kharin-2017 probabilistic calibration pipeline; see `references/methods.md`.
- `africas2s.metrics.crpss.{crps_normal, crps_climatology, crpss}` — Gaussian CRPS building blocks (ndarray in/out) behind the `crpss` metric.
- `africas2s.plotting.*` — see `references/plotting-reporting.md`.
