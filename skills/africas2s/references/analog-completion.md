...
## Analog selection (`africas2s.analog`)
...
## Climate positioning (`africas2s.climate`)
...
is anything `africas2s.time.season_bounds` accepts (`"JJAS"`, `(10, 2)`, timestamp pair).
...
## Scenario completion (`africas2s.completion`)
...
- `season`, `year` — the accumulation window (`season` per `africas2s.time.season_bounds`);
...
## Scalar-series calibration (`africas2s.series`)

Everything else in africas2s calibrates a *field*; these two calibrate a single number per year
...
(`africas2s.methods._qm_kernel`), so the two cannot disagree. `x` may be a scalar, array, or
...
## Calendar utilities (`africas2s.time`)

Reference these **module-qualified** — `time` is imported as a submodule but its functions are
**not** in the top-level `__all__` (use `africas2s.time.season_step`, not `ds.season_step`).

**Season-step alignment** — the coordinate every cross-year splice in africas2s joins on:
