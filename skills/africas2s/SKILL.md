...
# AfricaS2S — seasonal forecast downscaling, calibration, and verification

AfricaS2S turns coarse GCM seasonal hindcasts/forecasts plus fine-resolution observations into calibrated high-resolution forecasts (continuous fields or below/normal/above tercile probabilities) and scores them with cross-validated skill metrics. It operates purely on xarray and is source-agnostic — it pairs naturally with **rosetta** for data acquisition but does not require it.
...
from africas2s.tercile import to_tercile, to_tercile_cv
...
Any xarray source works; rosetta produces the exact shapes africas2s expects:
...
- Plotting/reporting/GeoTIFF need the `plotting` extra (`pip install africas2s[plotting]`); errors say so.
...
- [references/plotting-reporting.md](references/plotting-reporting.md) — which plot for which artifact, forecast/skill maps, panel grids & components+objective composites (`plot_matrix`), packaged colour languages (`TercileStyle.named`: `icpac` default, `icpac-temperature`, `icpac-onset`, `noaa-cpc`, `ghacof`, `acmad`) and workflow JSON style files (`TercileStyle.from_json`), dominance rules (`prob_bins[0]`, `secondary_max`), packaged classified field scales (`FieldScale.named`: `noaa-cpc-anomaly`, `ucsb-chirps-anomaly`, `ucsb-chirps-total`, `icpac-onset-date`, `icpac-onset-spread`; `scale=` on `plot_field`/`plot_matrix`), index-evolution plume (`plot_index_evolution`), field maps & choropleths, scenario/index plots, SVSLRF PDFs, GeoTIFF/NetCDF export, headless figure handling
