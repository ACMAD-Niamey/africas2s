# africas2s agent skill

An [Agent Skill](https://agentskills.io/) that teaches AI coding agents how to use the `africas2s` package: the core verbs (`downscale`, `optimize`, `calibrate`, `ensemble`, `skill`, `seasonal_mme`), method/metric/strategy registries, tercile leakage discipline, plotting/reporting, and rosetta integration.
...
ln -s ../../skills/africas2s .claude/skills/africas2s
```

**Claude Code (all your projects):** put it in your personal skills directory instead:

```bash
ln -s "$(pwd)/skills/africas2s" ~/.claude/skills/africas2s
```

Then Claude uses it automatically when a task involves africas2s, or invoke it explicitly with `/africas2s`.
...
```bash
skills-ref validate skills/africas2s   # from https://github.com/agentskills/agentskills
```
